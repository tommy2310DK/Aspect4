...
parser = argparse.ArgumentParser(description='Fetch order information from Aspect4 API')
parser.add_argument('customer_number', type=str, help='Customer number (mandatory)')
parser.add_argument('--order_number', type=str, help='Specific order number (optional)')
...
if args.start_date and args.end_date and args.days:
...
if args.start_date and args.end_date:
    ...
    try:
        min_dato = int(args.start_date)
        max_dato = int(args.end_date)
    except ValueError:
        print("Error: Dates must be in YYYYMMDD format", file=sys.stderr)
        sys.exit(1)
elif args.days:
    # Use days parameter
    past_date = now - timedelta(days=args.days)
...
if args.order_number:
    order_request['aordrenr'] = args.order_number
